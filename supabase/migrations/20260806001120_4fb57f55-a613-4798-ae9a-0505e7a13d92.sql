...
BEGIN
  IF NEW.reunion_datetime IS NULL THEN
    -- Caso antes no cubierto: hay fecha (y quizá hora) pero no instante completo.
    IF NEW.fecha_reunion IS NULL THEN
      RETURN NEW;
    END IF;
    v_local := NEW.fecha_reunion + COALESCE(NEW.hora_reunion, time '11:00');
  ELSE
    v_local := NEW.reunion_datetime AT TIME ZONE 'Europe/Madrid';
  END IF;

  -- 1) Fechas absurdas (epoch 1969, años lejanos) o pasadas -> siguiente dia laborable 11:00
  IF v_local < date_trunc('day', (now() AT TIME ZONE 'Europe/Madrid'))
